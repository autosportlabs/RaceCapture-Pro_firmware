...
#define MEM_START_ADDRESS 	{(void*)_heap_address}
#define MEM_END_ADDRESS     { ( void* )( _heap_address + _CONFIG_HEAP_SIZE - 1) }


extern unsigned int _CONFIG_HEAP_SIZE;
extern unsigned int _heap_address;

static void* platform_get_first_free_ram( unsigned id )
{
...
static void* platform_get_last_free_ram( unsigned id )
{
...
void* pvPortCalloc( size_t nmemb, size_t size )
{
  ...
  if( !s_initialized )
    s_init();
  if( ( ptr = pvPortMalloc( nmemb * size ) ) != NULL )
...
void* pvPortRealloc( void* ptr, size_t size )
{
  ...
  {
    if( ( newptr = pvPortMalloc( size ) ) == NULL )
      return NULL;
