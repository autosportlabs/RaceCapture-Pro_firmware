...
static void sendConfigurePositionUpdateRate(GpsMessage * gpsMsg,
					    Serial * serial, uint8_t updateRate)
{
		...
		while (1) {
			...
			if (baudRate) {
				...
				if ((baudRate != TARGET_BAUD_RATE) &&
				    configureBaudRate(gpsMsg, serial, TARGET_BAUD_RATE) == GPS_COMMAND_FAIL) {
				...
				if (!updateRate) {
					pr_error("GPS: Error provisioning - "
						 "could not detect update rate\r\n");
					updateRate = 0;
				}
				if ((updateRate != TARGET_UPDATE_RATE) &&
				    (configureUpdateRate(gpsMsg, serial, TARGET_UPDATE_RATE)) == GPS_COMMAND_FAIL) {
					pr_error("GPS: Error provisioning - "
						 "could not configure update rate\r\n");
					break;
				}

				if (configureNmeaMessages(gpsMsg, serial) == GPS_COMMAND_FAIL) {
					pr_error("GPS: Error provisioning - "
						 "could not configure NMEA messages\r\n");
					break;
				}
