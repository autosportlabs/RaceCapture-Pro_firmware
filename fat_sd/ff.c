...
/                   Added re-entrancy for multitasking operation.
...
)
{
...
#endif
		if (w < 0x80 && chk_chr("\"*:<>\?|\x7F", w)) /* Reject invalid chars for LFN */
			return FR_INVALID_NAME;
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	for (;;) {
		...
		if (!w) break;					/* Break when end of the LFN */
		...
		if (_DF1S && w >= 0x100) {		/* Double byte char */
			...
			if (!w || chk_chr("+,;[=]", w)) {	/* Replace invalid chars for SFN */
	...
	for (;;) {
		...
		if (IsDBCS1(c)) {				/* If it is DBC 1st byte */
			...
			if (chk_chr(" \"*+,[=]|\x7F", c))	/* Reject invalid chars for SFN */
				return FR_INVALID_NAME;
			if (IsUpper(c)) {
				b |= 2;
			} else {
...
)
{
	...
	if (*path == '/' || *path == '\\') { /* There is a heading separator */
		path++;	dj->sclust = 0;		/* Strip it and start from the root directory */
	} else {							/* No heading separator */
		dj->sclust = dj->fs->cdir;	/* Start from the current directory */
	}
#else
	if (*path == '/' || *path == '\\')	/* Strip heading separator if exist */
		path++;
	dj->sclust = 0;						/* Start from the root directory */
#endif
		...
		if (clst != 0) {
			while (ofs > bcs) {						/* Cluster following loop */
				...
				if (fp->flag & FA_WRITE) {			/* Check if in write mode or not */
					clst = create_chain(fp->fs, clst);	/* Force stretching if in write mode */
...
)
{
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Follow completed */
			dir = dj->dir;
			if (dir) {							/* It is not the root directory */
			...
			if (res == FR_OK) {
				dj->id = dj->fs->id;
				res = dir_seek(dj, 0);			/* Rewind directory */
			}
		}
		if (res == FR_NO_FILE) res = FR_NO_PATH;
	}
...
FRESULT f_getfree (
	const XCHAR *path,	/* Pointer to the logical drive number (root directory) */
...
)
{
	...
	if (dir[DIR_Attr] & AM_DIR) {			/* It is a sub-directory */
		...
		sdj.sclust = dclst;
		// patch for 0.07c from ChaN:
		// res = dir_seek(&sdj, 0);
		res = dir_seek(&sdj, 2);
		if (res != FR_OK) LEAVE_FF(dj.fs, res);
		res = dir_read(&sdj);
		if (res == FR_OK) res = FR_DENIED;	/* Not empty sub-directory */
...
)
{
	...
	if (!dj_old.dir) LEAVE_FF(dj_old.fs, FR_NO_FILE);	/* Is root directory? */
...
)
{
	...
	ST_WORD(tbl+BPB_RootEntCnt, SS(fs) / 32 * n_dir); /* Number of root-directory entries */
